...
####################################################################################################
#  
#  This file is from this repo: https://github.com/UjjwalSaxena/Automold--Road-Augmentation-Library
#
####################################################################################################
...
def m_add_blur(imgs, low_kernel=3, high_kernel=5, w_thresh=150, low_w_thresh=80):
    ...
    for img in imgs:
        ...
        if img_w < w_thresh:
            if img_w < low_w_thresh:
                img = cv2.blur(img, (2, 2))
            else:
                img = cv2.blur(img, (low_kernel, low_kernel))
        else:
            img = cv2.blur(img, (high_kernel, high_kernel))
...
def m_add_median_blur(imgs, low_kernel=3, high_kernel=5, w_thresh=150, low_w_thresh=80):
    if type(imgs) is not list:
        imgs = [imgs]
    ...
    for img in imgs:
        ...
        if img_w < w_thresh:
            if img_w < low_w_thresh:
                ...
                img = cv2.medianBlur(img, low_kernel)
        else:
            img = cv2.medianBlur(img, high_kernel)
...
def m_add_gaussian_blur(imgs, low_kernel=3, high_kernel=5, w_thresh=150, low_w_thresh=90, sigma=1):
    if type(imgs) is not list:
        imgs = [imgs]
    ...
    for img in imgs:
        ...
        if img_w < w_thresh:
            if img_w < low_w_thresh:
                img = cv2.GaussianBlur(img, (2,2), sigma)
            else:
                img = cv2.GaussianBlur(img, (low_kernel,low_kernel), sigma)
        else:
            img = cv2.GaussianBlur(img, (high_kernel, high_kernel), sigma)

        result.append(img)

    return result

def rand_rot(imgs, max_r=2):
    if type(imgs) is not list:
        imgs = [imgs]
    ...
    for img in imgs:
        img = rotate_bound(img, random.randint(-max_r, max_r))
        result.append(img)
